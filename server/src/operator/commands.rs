...
pub async fn get_session_details(
    ...
    let mut client = state.get_client().await.ok_or("Not connected")?;
    // Note: This would need a dedicated gRPC method, using placeholder
...
pub async fn cancel_task(
    ...
    let mut client = state.get_client().await.ok_or("Not connected")?;
    // Note: Would need dedicated gRPC method
    Err("Not implemented".to_string())
}
...
// Server Stats
#[tauri::command]
pub async fn get_server_stats(state: State<'_, AppState>) -> Result<ServerStats, String> {
    let mut client = state.get_client().await.ok_or("Not connected")?;
    client.get_server_stats().await.map_err(|e| e.to_string())
}
...
pub async fn upload_file(
    ...
    let mut payload = remote_path.into_bytes();
    payload.push(b'\n');
